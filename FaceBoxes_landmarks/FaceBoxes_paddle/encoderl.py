...
class DataEncoder:
    ...
    def encode(self, boxes, landmarks, classes, threshold=0.5):
        '''
        boxes:[num_obj, 4]
        default_box： [21824, 4] (x1,y1,w,h)
        return:boxes: [21824, 4]
        classes:class label [21824,]，都是1
        '''
        ...
        iou = get_iou(boxes, np.concatenate([x1y1, x2y2], 1))  # iou_size = (num_obj, 21824)
        ...
        boxes = boxes[face_index] #[21824, 4]
        ...
        tg_boxes = np.concatenate([cxcy, wh], axis=1)  # [21824,4]

        landmarks[:, ::2] -= np.repeat(np.expand_dims(default_boxes[:, 0], 1), 5, axis=1)  # [21824, 5]
        landmarks[:, ::2] /= variances * np.repeat(np.expand_dims(default_boxes[:, 2], 1), 5, axis=1)

        landmarks[:, 1::2] -= np.repeat(np.expand_dims(default_boxes[:, 1], 1), 5, axis=1)
        landmarks[:, 1::2] /= variances * np.repeat(np.expand_dims(default_boxes[:, 3], 1), 5, axis=1)

        return tg_obj, tg_boxes, landmarks

    def decode(self, conf, loc, landmarks):
        '''
        ...
        landmarks[:, ::2] *= variances* np.repeat(np.expand_dims(self.default_boxes[:, 2], 1), 5, 1)
        landmarks[:, ::2] += np.repeat(np.expand_dims(self.default_boxes[:, 0], 1), 5, 1)
        landmarks[:, 1::2] *= variances * np.repeat(np.expand_dims(self.default_boxes[:, 3], 1), 5, 1)
        landmarks[:, 1::2] += np.repeat(np.expand_dims(self.default_boxes[:, 1], 1), 5, 1)
        ...
        return boxes, landmarks, max_conf
        mask = max_conf > 0.9  # 筛选大于0.9的框
        return boxes[mask], landmarks[mask], max_conf[mask]
