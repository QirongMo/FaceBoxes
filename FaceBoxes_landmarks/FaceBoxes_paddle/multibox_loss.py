...
class MultiBoxLoss(paddle.nn.Layer):
	...
	def forward(self, loc_preds, loc_targets, landmarks_preds, landmarks_targets, conf_preds, conf_targets):
		'''
		...
		# got the loc loss
		pos = paddle.reshape(pos,(pos.shape[0],21824,1))
		...
		#get the landmarks loss
		pos_mask2 = paddle.nonzero(pos.expand_as(landmarks_preds))
		pos_landmarks_preds = paddle.gather_nd(landmarks_preds, pos_mask2).reshape((-1, 10))  # filter the pos face
		pos_landmarks_targets = paddle.gather_nd(landmarks_targets, pos_mask2).reshape((-1, 10))  # filter the pos face
		landmarks_criterion = paddle.nn.SmoothL1Loss()
		landmarks_loss = landmarks_criterion(pos_landmarks_preds, pos_landmarks_targets) * 10.  # 4 is get the sum of four loc
		...
		# conf_loss = cross_entropy(preds, targets, reduction='sum')
		# conf_loss /=  paddle.to_tensor(num_matched_boxes, dtype='float32', stop_gradient=False)
		conf_loss = paddle.nn.functional.softmax_with_cross_entropy(preds, targets)

		conf_loss = paddle.sum(conf_loss) / num_matched_boxes.astype('float32')
		return loc_loss, landmarks_loss, conf_loss, loc_loss+conf_loss+landmarks_loss
